...
def execute_named_search(
    ...
    """
    Execute a named area search (e.g., "lawyers in Midtown, Manhattan").

    This searches by area NAME rather than coordinates, which can yield
    different/more results from Google.

    Args:
        category: What to search for (e.g., "lawyers")
        area_name: Named area to search in (e.g., "Midtown, Manhattan")
        results_count: Number of results per page
        offset: Pagination offset
        timeout: Request timeout in seconds
    ...
    Returns:
        List of business dictionaries
    """
    # Build the search query with the area name
    search_query = f"{category} in {area_name}"

    # Use a default center point (Google will use the named area anyway)
    # We use NYC coordinates as a fallback but Google prioritizes the named location
    default_lat = 40.7128
    default_lng = -74.0060
    ...
    curl_command = build_search_curl(
        ...
        default_lng,
        results_count,
        DEFAULT_MAX_RADIUS * 2,  # Larger radius for named searches
        DEFAULT_VIEWPORT_DIST * 2,
        offset,
    )
