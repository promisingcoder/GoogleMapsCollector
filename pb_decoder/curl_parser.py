...
class CurlParser:
    ...
    def parse(self, curl_command: str) -> ParsedCurl:
        ...
        # Clean up the command - handle line continuations
    ...
    def _extract_url(self, command: str) -> str:
        ...
        # Try to find URL in quotes after 'curl'
        patterns = [
            r"curl\s+['\"]([^'\"]+)['\"]",  # curl 'url' or curl "url"
            r"curl\s+(\S+)",  # curl url (no quotes)
        ]
        ...
        # Alternative: find any https:// URL in the command
    ...
    def _extract_headers(self, command: str) -> Dict[str, str]:
        ...
        patterns = [
            r"-H\s+['\"]([^'\"]+)['\"]",  # -H 'header: value'
            r"-H\s+([^\s]+)",  # -H header:value (no quotes)
        ]
    ...
    def _extract_cookies(self, command: str) -> Dict[str, str]:
        ...
        # Find -b 'cookies' pattern
        patterns = [
            r"-b\s+['\"]([^'\"]+)['\"]",  # -b 'cookies'
            r"--cookie\s+['\"]([^'\"]+)['\"]",  # --cookie 'cookies'
        ]
...
if __name__ == "__main__":
    test_curl = """curl 'https://www.google.com/search?tbm=map&q=test' -H 'accept: */*' -H 'user-agent: Mozilla/5.0' -b 'NID=123; SID=456'"""

    result = parse_curl(test_curl)
